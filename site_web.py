...
def prix_float(prix_str):
    ...
    try: return float(re.sub(r'[^\d,]', '', prix_str).replace(',', '.'))
    except: return 0.0
...
async def traiter_message(message):
    ...
    for e in message.embeds: contenu += f" {e.description or ''} {e.title or ''} {e.url or ''}"
...
@client.event
async def on_ready():
    print(f"Bot connecté. Scan...")
    for s_id in SALONS_A_SURVEILLER:
        salon = client.get_channel(s_id)
        if salon:
            async for msg in salon.history(limit=50): await traiter_message(msg)
    print("Scan terminé.")
...
async def startup_event(): asyncio.create_task(client.start(TOKEN))
